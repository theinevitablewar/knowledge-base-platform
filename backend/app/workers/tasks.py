...
async def _run_with_engine_cleanup(coro) -> None:
    try:
        await coro
    finally:
...
def _run(coro) -> None:
    asyncio.run(_run_with_engine_cleanup(coro))
...
def ingest_task(self, task_id: str) -> None:
    ...
    try:
        _run(ingest_document(__import__("uuid").UUID(task_id)))
...
def delete_document_task(document_id: str) -> None:
    _run(delete_document_resources(__import__("uuid").UUID(document_id)))


@celery_app.task(name="knowledge_bases.cleanup")
def cleanup_knowledge_base_task(knowledge_base_id: str) -> None:
    _run(cleanup_knowledge_base(__import__("uuid").UUID(knowledge_base_id)))
