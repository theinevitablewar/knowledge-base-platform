...
export function LoginPage() {
  ...
  return <main className="login-page"><Card className="login-card"><div className="login-title"><span className="brand-mark large">K</span><Typography.Title level={2}>知识库平台</Typography.Title><Typography.Text type="secondary">登录后管理企业知识与 RAG 服务</Typography.Text></div>{error && <Alert type="error" showIcon message={error}/>}<Form layout="vertical" onFinish={submit}><Form.Item name="username" label="用户名" rules={[{required:true}]}><Input prefix={<UserOutlined/>} size="large"/></Form.Item><Form.Item name="password" label="密码" rules={[{required:true}]}><Input.Password prefix={<LockOutlined/>} size="large"/></Form.Item><Button htmlType="submit" type="primary" size="large" block loading={loading}>登录</Button></Form></Card></main>
}
